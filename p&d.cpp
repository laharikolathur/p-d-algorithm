...
{
printf("Process with process id P%d is already present inr the
queue.\n", pid);
}
}
nop = temp;
...
else if(dl >= bt + leastDeadline && dl <= largestDeadline -
largestDeadlineBT)
{
return 1;
}
...
for(; i < nop; i++)
{
printf("P%d\t\t%d\t\t%d\t\t%d\n", processes.processId[i],
processes.burstTime[i],
...
{
printf("P%d\t\t%d\t\t%d\t\t%d\n", dlQ.processId[i],
dlQ.burstTime[i], dlQ.priority[i], dlQ.deadline[i]);
}
...
{
printf("P%d\t\t%d\t\t%d\t\t%d\n", pQ.processId[i],
pQ.burstTime[i], pQ.priority[i], pQ.deadline[i]);
}
}
...
else if(elapsedTime + pQ.burstTime[PQIndex] +
dlQ.burstTime[DQIndex] <= dlQ.deadline[DQIndex])
{
printf("P%d\t", pQ.processId[PQIndex]);
elapsedTime += pQ.burstTime[PQIndex];
